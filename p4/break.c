 ...
 */
  ...
  rmp = mp;
  v = (vir_bytes) m_in.addr;/*new virtual address in D space  from message*/
  new_clicks = (vir_clicks) ( ((long) v + CLICK_SIZE - 1) >> CLICK_SHIFT);/*bytes 2 clicks*/
  ...
  if ((r=get_stack_ptr(who_e, &new_sp)) != OK) /* ask kernel for sp value */
  	panic(__FILE__,"couldn't get stack pointer", r);/*return value check*/
 ...
 */
  ...
  base_of_stack = (long) mem_sp->mem_vir + (long) mem_sp->mem_len;/*stack grow from top 2 down*/
...
#define SAFETY_CLICKS ((SAFETY_BYTES + CLICK_SIZE - 1) / CLICK_SIZE)
  ...
  if (lower < gap_base) {
    ...
    printf("allocate_new_mem succeed!\n");
  ...
  if (r == OK) {
	...
	if (changed && (r2=sys_newmap(rmp->mp_endpoint, rmp->mp_seg)) != OK)
  		panic(__FILE__,"couldn't sys_newmap in adjust", r2);/*original sys_newmap here*/
	return(OK);
  printf("sysnewmap fit\n");
  }
...
PUBLIC int allocate_new_mem(rmp,memory_clicks)
register struct mproc *rmp;
phys_clicks memory_clicks;

{
  register struct mem_map *mem_sp, *mem_dp;
  int s;/*return value check*/
  phys_clicks old_data_base,old_stack_base,new_data_base,new_stack_base;
  phys_bytes data_bytes,stck_bytes;
  phys_bytes old_data_abs,new_data_abs,old_stack_abs,new_stack_abs;
  phys_clicks old_base_pointer,new_base_pointer;
  phys_clicks old_memory_size,new_memoey_size;
  phys_clicks now_data_base,now_stack_base,now_stackV_base,now_data_size,now_stack_size;
  vir_clicks sp_click;
  mem_dp = &rmp->mp_seg[D];	
  mem_sp = &rmp->mp_seg[S];
  old_memory_size=memory_clicks;
  new_memoey_size =2*memory_clicks;
  
  new_base_pointer=alloc_mem(new_memoey_size);
  ...
  new_stack_base=new_base_pointer+new_memoey_size-(phys_clicks)mem_sp->mem_len;/*vir_clicks*/
  ...
  if ((s=sys_memset(0,new_data_abs,(new_memoey_size<<CLICK_SHIFT)))!=OK){   
        panic(__FILE__,"new mem can't be zero",s);   
  }
  #define NONE 0
  s=sys_abscopy(old_data_abs,new_data_abs,data_bytes);
  if (s < 0 ) panic(__FILE__,"allocate_new_mem can't copy",s);
  s=sys_abscopy(old_stack_abs,new_stack_abs,stck_bytes);
  if (s < 0 ) panic(__FILE__,"allocate_new_mem can't copy",s);
  /*copy adjust function*/
  /*save in case to restore under size not fit occasion*/
  /*now_data_base=mem_dp->mem_phys;
  now_stack_base=mem_sp->mem_phys;
  now_stack_size=mem_sp->mem_len;
  now_stackV_base=mem_sp->mem_vir;
  now_data_size=mem_dp->mem_len;*/
  
  /*update data and stack*/
  /*mem_dp->mem_vir stays the same*/
  /*mem_dp->mem_phys = new_data_base;/*does pointer change directly?->yep*/
	/*mem_sp->mem_phys = new_stack_base;
  mem_sp->mem_vir = mem_dp->mem_vir+new_memoey_size-mem_sp->mem_len;*/
  rmp->mp_seg[D].mem_phys = new_data_base;   
  rmp->mp_seg[S].mem_phys = new_stack_base;   
  rmp->mp_seg[S].mem_vir = mem_dp->mem_vir+new_memoey_size-mem_sp->mem_len;/**/
  /*if (!(rmp->mp_flags & SEPARATE))   
      rmp->mp_seg[T].mem_phys = new_data_base;*/
