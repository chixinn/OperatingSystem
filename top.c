...
void parse_file(pid_t pid)
{
	...
	sprintf(path, "%d/psinfo", pid);//写入path
...
void print_procs(int maxlines,
	struct proc *proc1, struct proc *proc2, int cputimemode)//maxlines不需要。
{
	int p, nprocs;
	u64_t idleticks = 0;
	u64_t kernelticks = 0;
...
#define NLOADS 3
	...
	lines += print_proc_summary(proc);//2
	lines += print_memory();
	...
	print_procs(r - lines - 2, prev_proc, proc, cputimemode);//3
